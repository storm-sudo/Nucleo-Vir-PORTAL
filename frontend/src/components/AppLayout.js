  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-900 transition-colors">
        ...
        <div className="px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-16">
            <div className="flex items-center space-x-4">
              <button onClick={() => setSidebarOpen(!sidebarOpen)} className="lg:hidden text-slate-700 dark:text-slate-200">
                {sidebarOpen ? <X className="h-6 w-6" /> : <Menu className="h-6 w-6" />}
              </button>
              <h1 className="text-lg font-heading font-bold text-slate-900 dark:text-white">Nucleo-vir Therapeutics</h1>
            </div>
            <div className="flex items-center space-x-4">
              <ThemeToggle />
              {user && (
                <div className="text-sm text-right hidden sm:block">
                  ...
                  <div className="text-slate-500 dark:text-slate-400 text-xs">{user.role}</div>
                </div>
              )}
              <Button variant="outline" size="sm" onClick={handleLogout} data-testid="logout-btn" className="dark:border-slate-600 dark:text-slate-200 dark:hover:bg-slate-700">
      ...
      <div className="flex">
        ...
        } lg:translate-x-0 fixed lg:static inset-y-0 left-0 z-30 w-64 bg-white dark:bg-slate-800 border-r border-slate-200 dark:border-slate-700 transition-transform duration-300 ease-in-out overflow-y-auto`}>
